...
void Forgetpass::loginButtonClk()
{
...
void Forgetpass::closeTheWarning()
{
    timer1->stop();
    ui->warning->hide();
    ui->warning_1->hide();
...
void Forgetpass::checkIfChangeVeriCodeButtom(const QString& text)
{
...
void Forgetpass::checkIfChangeLoginButtom(const QString& text)
{
    qDebug() << "checkIfDisplayLoginBottum" << endl;
...
void Forgetpass::areanumButtonClicked(QAction *action)
{
...
void Forgetpass::displayTheRemainingTime()
{
    if(time == -1){
        time = 60;
        timer->stop();
